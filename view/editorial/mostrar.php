			...
			<div class="container-fluid">
				<ul class="full-box list-unstyled page-nav-tabs">
					<li>
						...
						<a class="active" href="#"><i class="fas fa-clipboard-list fa-fw"></i> &nbsp; LISTA DE EDITORIALES</a>
					</li>
					<li>
						<a href="nuevo.php"><i class="fas fa-plus fa-fw"></i> &nbsp; AGREGAR EDITORIALES</a>
					</li>
...
<?php if(count($data)>0):?>
	...
	<table class="table table-dark table-sm">
		...
		<tr class="text-center">
			<td><?php echo $d->id_edi; ?></td>
			<td><?php echo $d->nomedi; ?></td>
			<td>
		 <?php if($d->estado==1)  { ?> 
		 
		<form  method="get" action="javascript:activo('<?php echo $d->id_edi; ?>')">
		<strong>	<button type="submit" class="badge badge-success">Activo</strong></button>
		</form>
			<?php  }   else {?> 

			 <form  method="get" action="javascript:inactivo('<?php echo $d->id_edi; ?>')"> 
				<strong><button type="submit" class="badge badge-danger">Inactivo</strong></button>
			</form>
		<?php  } ?>                         
			</td>
			<td>
		<a href="#edit_<?php echo $d->id_edi; ?>" title="Editar datos" data-backdrop="false" class="btn btn-primary btn-sm" data-toggle="modal"><span class="fas fa-edit" aria-hidden="true"></span></a>
		<a href="#delete_<?php echo $d->id_edi; ?>" title="Eliminar" data-backdrop="false" class="btn btn-danger btn-sm" data-toggle="modal"><span class="fas fa-trash"></span></a>
	...
	<script src="../../assets/js/reloj.js"></script>
		<script>
	function activo(id_edi)
{
	var id=id_edi;
	$.ajax({
        type:"GET",
		url:"../../assets/ajax/editar_estado_activo_editorial.php?id="+id,
    }).done(function(data){
...
function inactivo(id_edi)
{
	var id=id_edi;
	$.ajax({
		type:"GET",
		url:"../../assets/ajax/editar_estado_inactivo_editorial.php?id="+id,
    }).done(function(data){
        window.location.href ='mostrar.php';
    })
}
